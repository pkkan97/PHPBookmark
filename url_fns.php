...
function recommend_urls()
{
 ...
 $query="
select x.bm_URL from (
select count(a.bm_URL) counth, c.bm_url
from bookmark a, user b,bookmark c where a.username=b.username and a.username=c.username
group by c.bm_url
ORDER by counth desc limit 4) x";
	
	
	
	/*$query = "select max(count(bm_URL))
	    from bookmark
	    where username in
